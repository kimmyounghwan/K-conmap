...
def get_hybrid_1st_bids():
    ...
    try:
        headers = {"User-Agent": "Mozilla/5.0"}
        params = {
            'serviceKey': SAFE_API_KEY, 'numOfRows': '999', 'pageNo': '1',
            'inqryDiv': '1',
    ...
    for item in api_items:
        try:
            ...
            if len(corp) > 1 and corp[0].strip():
                new_rows[bid_no] = {
...
def get_hybrid_live_bids():
    ...
    for item in api_items:
        ...
        new_rows[bid_no] = {
            ...
            '발주기관': item.get('ntceInsttNm', ''), '예산금액': int(float(item.get('bdgtAmt', 0) or 0)),
        ...
        if st.button("로그인"):
            ...
            if not le_clean or not lp:
                ...
                try:
                    ...
                    st.toast("로그인 실패 🥲 이메일이나 비밀번호를 다시 확인해 주세요.", icon="🚨")

                # [해결 핵심!] 에러 그물망(try-except)을 벗어난 안전지대에서만 새로고침 실행!
                if login_success:
                    st.rerun()
