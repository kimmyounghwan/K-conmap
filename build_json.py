...
#      실측: 화면에 뜨는 단추 20,055개 중 **8,768개(43.7%)가 빈 페이지로** 갔습니다.
#      (소장님: 「대유건설도 3년 동안 1순위 1번 했는데 왜 없지?」 — 박윤정 대표 건이 그것이었습니다)
#      → 1 로 내려 «한 번이라도 1순위를 한 법인» 은 전부 자기 칸을 갖게 합니다.
#      실측 비용: 업체 57,694 → 67,763곳. 새로 생긴 1건짜리 법인 칸 10,062개가 raw 7.1MB.
#      묶음(CHUNK)은 200곳씩이라 «파일 수»가 늘 뿐 한 사람이 받는 양은 그대로입니다 —
#      저장용량만 늘고 전송량은 안 늘어납니다. names.json 에서 법인 키를 빼
#      gzip 348KB → 263KB 로 줄어 일부는 상쇄됩니다.
#      ⚠️ 처음에 「단추 49,091개 중 37,789개」라고 셌는데 **틀렸습니다** —
#         화면(mixbox)은 bzn>1 일 때만 뜨는데 그 조건을 빼고 셌습니다. 위가 맞는 숫자입니다.
#      (CLAUDE.md 9절 「corp/dat 다이어트 안 함 — 업체를 쳤는데 없습니다가 늘어 손해가 큽니다」)
...
def build_corp(df):
    ...
    for key, g in d2.groupby("ckey", sort=False):
        ...
        # ★ 순위 기록
        #   🚨 2026-09-16 — 예전에는 «번호로 못 찾으면 이름으로» 되돌렸습니다.
        #      그 되돌림이 **번호로 갈라 놓은 법인 칸(이름#번호)에서도** 돌아서,
        #      같은 이름 «전체»의 순위가 그 법인 성적으로 실렸습니다.
        #      실측: 갈라 놓은 11,735곳 중 5,053곳(43%)이 남의 기록을 달고 있었습니다.
        #      «정도건설#3048105833» 과 «정도건설#4088158893» 이 «똑같은 6건»을 달았습니다.
        #      그런데 화면은 그 위에 「이 법인 하나만의 기록입니다」라고 적고 있었습니다.
        #      (소장님이 「1순위 한번인데 총낙찰은 2번이야」를 보고 잡아내셨습니다)
        #      → 칸이 «무엇이라고 주장하는지»에 맞춥니다:
        #         · 법인 칸(이름#번호) = 그 사업자번호 기록만. 없으면 «없다»고 적습니다.
        #         · 이름 칸           = 이름 기록(여러 법인 합계). 화면이 이미 그렇게 경고합니다.
        #      rkby 는 화면이 «무엇을 보고 있는지» 적어 주기 위한 표시입니다.
        if is_sub:
            _recs = rk_biz.get(key.split("#", 1)[1], [])
            _by = "biz"
        else:
            _recs = rk_name.get(key, [])
            _by = "name"
        ...
        if _recs:
            ...
            cur[key]["rkby"] = _by
        # ★ 자주 만나는 상대 — 맞대결 표가 «이름» 단위입니다(load_rank_history 의 met).
        #   그래서 법인 칸에는 싣지 않습니다. 실으면 또 남의 맞대결이 됩니다.
        if not is_sub:
    ...
    #   ⚠️ 2026-09-16 — «이름#번호» 키는 뺍니다. 이 파일은 «이름 가운데로 찾기» 용인데,
    #      법인 키는 이름 키가 이미 덮습니다. 넣어 두면 파일만 커집니다
    #      (CORP_MIN_SPLIT 을 1 로 내리면서 4만 줄이 더 들어올 뻔했습니다).
