...
with st.sidebar:
    ...
    st.write("---")
    st.info("💡 최초 1회 로딩 시 조달청 서버 상황에 따라 최대 100초 정도 걸릴 수 있습니다.")
...
if menu == "📊 실시간 공고 (홈)":
    if 'master_data' not in st.session_state:
        # 🚨 스피너 문구 수정: 100초 기다림 안내
        with st.spinner("조달청 창고에서 공고를 꺼내오는 중입니다... 서버가 느려서 최대 100초 정도 걸릴 수 있어요! ⏳"):
    ...
    if not df.empty:
        ...
        with col1:
            st.metric(label="누적 공고(최근 30일)", value=f"{len(df):,}건")
        with col2:
