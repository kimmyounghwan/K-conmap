...
function BidTab({ r }) {
  ...
  return (
    <>
      ...
      {(r.corps || []).length === 1 && (
        <div className="hintbox">
          {/* ⚠️ 실측(2026-09-02): 개찰결과 API 의 opengCorpInfo 에는 업체가 «한 곳만» 옵니다.
              참가업체수가 23곳인 공고에서도 1곳이었고, 쌓인 10,913건 전부 1곳이었습니다.
              그래서 «안 준다» 가 아니라 «이 오퍼레이션은 안 준다» 가 정확한 말입니다. */}
          조달청 <b>공개 API 는 1순위(낙찰자)만</b> 줍니다 — 참가업체수가 수십 곳이어도
          한 곳만 실려 옵니다(개찰 10,913건 전수 확인).
          {r.np > 1 && <> 이 공고에는 <b>{r.np}곳</b>이 들어왔습니다.</>}
          <br />
          전체 투찰내역은 조달청이 <b>따로</b> 공개합니다 —
          공공데이터포털 «입찰공고 기업별 투찰 및 계약내역».
          그 파일을 받아 넣으면 이 자리에 1위부터 10위까지 나옵니다.
        </div>
      )}
