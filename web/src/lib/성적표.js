...
export function 세해요약(c) {
  ...
  return {
    ...
    /* 🗓 2026-09-19 — 소장님: 「자료가 풍부할 수록 좋은 거지. 업체분석이니까」
       3년치 자료에 이미 들어 있는데 종이에 안 쓰던 것들입니다. 새로 세지 않고 «그대로» 씁니다. */
    투찰률분포: (c.h || []).slice().sort((a, b) => a[0] - b[0]),   // [[구간, 건수]]
    달마다: Array.isArray(c.m) && c.m.length === 12 ? c.m.slice() : null,
    /* 이름이 같고 사업자번호가 다른 법인 — 1,846가지 있습니다. 헷갈리면 성적표가 통째로 엉뚱해집니다 */
    같은이름: { 몇: c.bzn || 0, 목록: (c.bz || []).slice(0, 6) },
    최근순위: (c.rank || []).slice(0, 6),
  }
}
