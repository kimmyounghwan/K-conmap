...
class 그림판 {
  ...
  /** 칸 너비를 넘치면 잘라내고 «…» 을 붙입니다 — 넘쳐서 옆 칸을 덮는 것보다 낫습니다 */
  자르기(segs, pt, maxW) {
    const 재기 = (arr) => arr.reduce((a, b) => a + b.w, 0)
    let out = segs.map((x) => { this.글꼴설정(pt, x.b); return { ...x, w: this.x.measureText(x.s).width } })
    if (재기(out) <= maxW) return out
    this.글꼴설정(pt, false)
    const 점 = this.x.measureText('…').width
    while (out.length && 재기(out) + 점 > maxW) {
      const last = out[out.length - 1]
      const t = [...last.s]
      t.pop()
      if (!t.length) { out.pop(); continue }
      this.글꼴설정(pt, last.b)
      last.s = t.join('')
      last.w = this.x.measureText(last.s).width
    }
    out.push({ s: '…', b: false, c: 흐림, w: 점 })
    return out
  }
  선(y, x0, x1, 색 = 줄색, w = 1) {
...
function 네모표(g, head, rows, 너비몫) {
  ...
  for (const r of rows) {
    ...
    r.forEach((cell, i) => {
      g.칸(x, y, cw[i], rh, { 테두리: 줄색 })
      const 줄 = [g.자르기(조각(String(cell)), pt, cw[i] - PT(18))]
      const tw = 줄[0].reduce((a, b) => a + b.w, 0)
...
function 기록표(g, head, rows, 너비몫) {
  ...
  for (const r of rows) {
    ...
    r.forEach((cell, i) => {
      ...
      const 줄 = [g.자르기(segs, pt, cw[i] - PT(9))]
...
/* ══════════════════════════════════════════════════════════════
   ...
   ⚠️ 위쪽 칸들과 «자료가 다릅니다». 여기는 3년치 «낙찰(1순위)» 기록이고,
      떨어진 것은 최근 두 달치에만 있습니다. 종이에 그 사실을 적어 둡니다 —
      안 적으면 3년 내내 떨어진 기록이 있는 것처럼 읽힙니다.
   ⚠️ 새 식을 여기 적지 않습니다. 성적표.js 의 세해요약() 이 묶어 둔 것을 그리기만 합니다.
   ══════════════════════════════════════════════════════════════ */
function 세해칸(g, 세, fn, fi, f, 가릴까표시) {
  if (!세 || !세.낙찰) return
  표제(g, '3년치 낙찰 기록 — 어디서 따 왔나', true)
  g.글('여기부터는 *3년치 개찰의 «1순위(낙찰)» 기록* 입니다. 위쪽 «넣은 것 전부» 와 자료가 다릅니다 — ' +
       '떨어진 기록은 최근 두 달치부터 쌓이고 있습니다.', { pt: 8.5, 색: 흐림 })
  타일(g, [
    { k: '3년치 낙찰', v: `${세.낙찰}건`, s: 세.기간 ? `${세.기간[0]} ~ ${세.기간[1]}년` : '' },
    { k: '평균 투찰률', v: 세.투찰률.평균 == null ? '—' : `${세.투찰률.평균}%`,
      s: 세.투찰률.중앙 == null ? '' : `중앙 ${세.투찰률.중앙}%` },
    { k: '가장 낮게', v: 세.투찰률.최저 == null ? '—' : `${세.투찰률.최저}%`,
      s: 세.투찰률.최고 == null ? '' : `가장 높게 ${세.투찰률.최고}%` },
    { k: '평균 낙찰금액', v: 돈(세.금액.평균),
      s: 세.금액.최고 == null ? '' : `가장 큰 것 ${돈(세.금액.최고)}` },
  ])
  if (세.해마다 && 세.해마다.length) {
    const mx = Math.max(...세.해마다.map((x) => x.건)) || 1
    막대(g, 세.해마다.map((x) => ({ k: `${x.해}년`, pct: (x.건 / mx) * 100, v: `${x.건}건` })), 60)
  }
  if (세.지역 && 세.지역.length) {
    g.글('지역 — ' + 세.지역.map(([k2, v]) => `${fi(k2)} ${v}건`).join(' · '),
         { pt: 9.5, 위: PT(8) })
  }
  if (세.기관 && 세.기관.length) {
    표제(g, '어느 기관에서 땄나 (3년치)')
    네모표(g, ['발주기관', '낙찰'], 세.기관.map(([nm, c]) => [fi(nm), `${c}건`]), [0.78, 0.22])
  }
  /* 3년치 낙찰이 «어느 투찰률» 에 몰렸나 — 겨냥하는 자리가 보입니다 */
  if (세.투찰률분포 && 세.투찰률분포.length) {
    표제(g, '3년치 — 어느 투찰률에서 땄나')
    const mx = Math.max(...세.투찰률분포.map((x) => x[1])) || 1
    막대(g, 세.투찰률분포.map(([구간, 건]) => ({
      k: `${Number(구간).toFixed(1)}%대`, pct: (건 / mx) * 100, v: `${건}건`,
    })), 66)
    g.글('낙찰된 건들이 «실제로 어느 자리» 에서 나왔는지입니다. 한 자리에 몰려 있으면 ' +
         '그 자리가 이 업체의 겨냥점입니다.', { pt: 8.5, 색: 흐림, 위: PT(6) })
  }
  /* 달마다 — 일이 몰리는 철 */
  if (세.달마다 && 세.달마다.some((v) => v)) {
    표제(g, '3년치 — 어느 달에 땄나')
    const mx = Math.max(...세.달마다) || 1
    막대(g, 세.달마다.map((v, i) => ({ k: `${i + 1}월`, pct: (v / mx) * 100, v: `${v}건` })), 44)
  }
  if (세.사례 && 세.사례.length) {
    표제(g, '최근 낙찰 (3년치)')
    네모표(g, ['공고', '개찰일', '투찰률', '낙찰금액'],
      세.사례.map((c) => [fn(c[0] || ''), String(c[1] || '').slice(0, 10),
        c[3] == null ? '—' : `${c[3]}%`, 돈(c[4])]),
      [0.44, 0.16, 0.14, 0.26])
  }
  /* 떨어진 것이 섞인 «최근 순위 기록» — 사이트가 쌓기 시작한 만큼만 있습니다 */
  if (세.최근순위 && 세.최근순위.length) {
    표제(g, '최근 순위 기록 — 떨어진 것까지')
    네모표(g, ['공고', '개찰일', '내 등수', '참가', '투찰률'],
      세.최근순위.map((c) => [fn(c[0] || ''), String(c[1] || '').slice(0, 10),
        c[3] == null ? '—' : `${c[3]}위`, c[4] == null ? '—' : `${c[4]}곳`,
        c[5] == null ? '—' : `${c[5]}%`]),
      [0.40, 0.16, 0.14, 0.13, 0.17])
    g.글('이 칸은 «넣었는데 떨어진 것» 까지 들어 있습니다. 조달청이 투찰업체 목록을 주기 ' +
         '시작한 뒤의 개찰만이라 아직 몇 건뿐입니다 — 앞으로 쌓입니다.',
         { pt: 8.5, 색: 흐림, 위: PT(6) })
  }
  /* 이름이 같은 다른 법인 — 성적표가 엉뚱한 회사 것이 되지 않게 */
  if (세.같은이름 && 세.같은이름.몇 > 1 && 세.같은이름.목록.length) {
    표제(g, '⚠️ 이름이 같은 다른 법인')
    g.글(`«${f(세.이름)}» 이라는 이름으로 *${세.같은이름.몇}개 법인* 이 잡힙니다. ` +
         '이 성적표는 아래 첫 줄(사업자등록번호)의 법인 것입니다.', { pt: 9.5 })
    네모표(g, ['사업자등록번호', '대표', '3년 낙찰'],
      세.같은이름.목록.map((c) => [가릴까표시(c[0]), f(c[1] || ''), `${c[2] || 0}건`]),
      [0.44, 0.3, 0.26])
  }
}

/* 최근 두 달에 넣은 것이 «한 건도» 없을 때 — 3년치만으로 내는 종이 */
function 세해만종이(d, 가릴까) {
  const f = 가릴까 ? 이름가리기 : ((s2) => 씻기(s2))
  const fn = 가릴까 ? ((s2) => 씻기(공고가리기(s2))) : ((s2) => 씻기(s2))
  const fi = (s2) => 씻기(s2)
  const co = d.업체
  ...
  /* 🔖 2026-09-19 — 소장님: 「대표이름도 나오게 해줘」. 견본(가리기)에서는 이름도 가립니다. */
  const 대표 = co.대표 ? `    대표 ${가릴까 ? '○○○' : co.대표}` : ''
  g.글(`*${f(co.이름)}* 귀중` + (bno ? `    사업자등록번호 ${가릴까 ? `${bno.slice(0, 3)}-**-*****` : bno}` : '') + 대표,
       { pt: 12, 줄간: 1.4 })
  상자글(g, '최근 두 달 개찰에는 *넣으신 기록이 없습니다.* 그래서 «왜 떨어졌나 · 분위 · 놓친 자리» 는 ' +
            '이 종이에 없습니다. 아래는 *3년치 낙찰 기록* 입니다.',
         { 배경: '#fff7ed', 테: 녹, pt: 9.5 })
  세해칸(g, d.세해, fn, fi, f, (b2) => (가릴까 ? `${String(b2).slice(0, 3)}-**-*****` : String(b2)))
  꼬리(g, '이 성적표는 *K-건설맵*이 만들어 *무료로* 드린 것입니다. *k-conmap.com* 에서 ' +
    ...
    ['조달청이 공개한 개찰 결과를 정리한 것입니다. 따로 캐낸 자료가 아니며, 낙찰을 보장하지 않습니다.',
     '떨어진 기록(투찰업체 목록)은 최근치부터 쌓입니다. 그 앞은 1순위(낙찰)만 있습니다.',
...
export function 그리기(d, { 가릴까 = false } = {}) {
  if (d && d.두달없음) return 세해만종이(d, 가릴까)
  ...
  /* 기록: 60건까지 · _전부: 전부. 종이는 «전부» 를 봅니다 — 자가진단(화면)이 «딴 것» 만
     보여 주는 데 견줘, 성적표는 «넣은 것 전부» 를 놓고 말하는 종이여야 하기 때문입니다. */
  const R = d._전부 || d.기록 || []
  ...
  g.글('⟦K-건설맵⟧   k-conmap.com', { pt: 9 })
  g.글('입찰 성적표', { pt: 21, 위: PT(4), 줄간: 1.3 })
  g.글(`*${f(co.이름)}* 귀중    사업자등록번호 ${bno_s}` +
       (co.대표 ? `    대표 ${가릴까 ? '○○○' : co.대표}` : ''), { pt: 12, 줄간: 1.4 })
  ...
  /* ── 달마다 ─────────────────────────────────────────────
     «언제 많이 넣었나 · 그 달에 땄나». 화면(자가진단)은 낙찰한 달만 셉니다 —
     여기서는 «넣은 달» 을 같이 놓아야 «헛심 쓴 달» 이 보입니다. */
  const 달 = 묶어세기(R, (x) => String(x.dt || '').slice(0, 7))
  if (달.size >= 2) {
    표제(g, '달마다 — 언제 많이 넣었고, 언제 땄나')
    const 달차례 = [...달.keys()].sort()
    const 달최다 = Math.max(...[...달.values()].map((v) => v.투찰)) || 1
    막대(g, 달차례.map((k) => {
      const v = 달.get(k)
      return { k: `${k.slice(0, 4)}년 ${Number(k.slice(5, 7))}월`, pct: v.투찰 / 달최다 * 100,
               v: `${v.투찰}건${v.낙찰 ? ` · 낙찰 ${v.낙찰}` : ''}` }
    }), 92)
    g.글('막대는 «넣은 건수» 입니다. 오른쪽 숫자 뒤에 «낙찰» 이 붙은 달이 딴 달입니다.',
         { pt: 8.5, 색: 흐림, 위: PT(6) })
  }
  ...
  /* ── 금액을 조금 움직였다면 ────────────────────────────
     성적표가 «지난 일 정리» 로 끝나지 않게 하는 칸입니다.
     ⚠️ 남들 금액은 그대로라고 보고 센 값입니다 — 그 전제를 표 밑에 반드시 적습니다. */
  const 되짚칸 = [-0.4, -0.3, -0.2, -0.1, 0, 0.1, 0.2, 0.3, 0.4]
    .map((x) => ({ d: x, ...되짚기(R, x) })).filter((x) => x.잰것)
  if (되짚칸.length && 되짚칸[0].잰것 >= 5) {
    표제(g, '금액을 조금 움직였다면 — 몇 건이 달라졌나', true)
    g.글('넣으신 금액 전부를 예정가격의 *몇 %p 씩* 위아래로 옮겨 놓고 다시 세어 본 것입니다. ' +
         `잰 개찰 *${되짚칸[0].잰것}건*. *0.000%p 줄이 지금 성적*입니다.`, { pt: 10.5 })
    네모표(g, ['금액을 이만큼', '1순위', '하한 미달(실격)', '남는 자리'],
      되짚칸.map((x) => {
        const 지금 = x.d === 0
        const 나 = `${x.d >= 0 ? '+' : ''}${x.d.toFixed(1)}%p`
        return [지금 ? `*${나} (지금)*` : 나,
                x.일순위 ? `⟦${x.일순위}건⟧` : '0건',
                x.실격 ? `⟨${x.실격}건⟩` : '0건',
                `${x.잰것 - x.일순위 - x.실격}건`]
      }), [0.34, 0.22, 0.26, 0.18])
    g.글('⚠ *내가 금액을 바꿨다고 남들 금액까지 바뀌지는 않는다*는 가정 위에서 센 값입니다. ' +
         '실제 개찰에서는 남도 움직입니다. 그러니 이 표는 «이렇게 하면 이만큼 딴다» 가 아니라 ' +
         '*내 금액이 낙찰선에서 얼마나 떨어져 있었나*를 보는 자로 읽으십시오. ' +
         '숫자를 낮출수록 1순위가 조금 늘고 실격은 그보다 훨씬 빨리 늘어난다면, ' +
         '그건 이미 겨냥이 제자리라는 뜻입니다.', { pt: 8.5, 색: 흐림, 위: PT(6) })
  }
  ...
  /* ── 참가가 몇 곳일 때 ───────────────────────────────────
     3년치 실측(8,406건): 금액(분위)을 어떻게 잡아도 1순위율은 3.5~4.4% 에서 안 움직이는데,
     참가업체 수로는 18.2%(2~9곳) ~ 1.6%(100곳+) 로 갈립니다.
     **승부를 가르는 것은 금액이 아니라 어느 공고에 넣느냐** — 그 말을 이 회사 숫자로 합니다. */
  const 참가 = 묶어세기(R, (x) => 참가칸(x.n))
  if (참가.size >= 2) {
    표제(g, '참가가 몇 곳일 때 어땠나', true)
    g.글('*승부를 가르는 것은 금액이 아니라 «어느 공고에 넣느냐»입니다.* ' +
         '3년치 개찰 8,406건에서, 참가 2~9곳이면 1순위율 18.2%인데 100곳이 넘으면 1.6%입니다. ' +
         '귀사 기록을 같은 잣대로 갈라 봅니다.', { pt: 10.5 })
    네모표(g, ['참가업체', '투찰', '1순위', '실격', '평균등수', '1순위율'],
      칸차례.filter((k) => 참가.has(k)).map((k) => {
        const v = 참가.get(k)
        return [k, `${v.투찰}건`, v.낙찰 ? `⟦${v.낙찰}⟧` : '0', v.실격 ? `⟨${v.실격}⟩` : '0',
                `${(v.등수합 / v.투찰).toFixed(1)}위`, `${(v.낙찰 / v.투찰 * 100).toFixed(0)}%`]
      }), [0.28, 0.14, 0.13, 0.13, 0.16, 0.16])
    const 적은칸 = ['혼자 (경쟁 없음)', '2~9곳'].filter((k) => 참가.has(k))
    const 적은투 = 적은칸.reduce((a, k) => a + 참가.get(k).투찰, 0)
    const 많은칸 = ['30~99곳', '100곳 이상'].filter((k) => 참가.has(k))
    const 많은투 = 많은칸.reduce((a, k) => a + 참가.get(k).투찰, 0)
    const 많은낙 = 많은칸.reduce((a, k) => a + 참가.get(k).낙찰, 0)
    if (많은투 >= 3 && 많은투 > 적은투) {
      상자글(g, `투찰 ${R.length}건 가운데 *${많은투}건*이 «30곳 넘게 붙는 자리»였습니다. ` +
             `거기서 딴 것은 ${많은낙}건입니다. 사람이 많이 붙는 자리는 금액을 아무리 다듬어도 ` +
             '확률이 올라가지 않습니다 — *사람이 덜 붙는 자리를 찾는 것*이 먼저입니다.',
             { 배경: '#eff4ff', 테: 파랑 })
    }
    g.글('1순위율은 그 칸에서만 센 것입니다. 건수가 적은 칸은 참고만 하십시오.',
         { pt: 8.5, 색: 흐림, 위: PT(6) })
  }

  /* ── 시·도별 ───────────────────────────────────────────
     ⚠️ 기관 이름으로 시·도를 짐작하면 안 됩니다 — «김천시산림조합» 에는 도 이름이 없습니다.
        조달청이 준 «공사 지역»(site) 을 씁니다. 그것도 없으면 그 줄은 셈에서 뺍니다. */
  const 지역 = 묶어세기(R, (x) => (x.site ? 시도(x.site) : ''))
  if (지역.size >= 2) {
    표제(g, '어느 시·도에 냈나')
    네모표(g, ['시·도', '투찰', '낙찰', '실격', '평균등수'],
      [...지역.entries()].sort((a, b) => b[1].투찰 - a[1].투찰).slice(0, 10).map(([k, v]) => [
        k, `${v.투찰}건`, v.낙찰 ? `⟦${v.낙찰}⟧` : '0', v.실격 ? `⟨${v.실격}⟩` : '0',
        `${(v.등수합 / v.투찰).toFixed(1)}위`,
  ...
  /* ── 딴 공사 ─────────────────────────────────────────── */
  const 딴것 = R.filter((r) => r.rank === 1)
  if (딴것.length) {
    표제(g, `딴 공사 ${딴것.length}건`, true)
    기록표(g, ['개찰', '공고', '참가', '낙찰금액', '투찰률', '하한 위'],
      딴것.map((r) => [
        { s: String(r.dt).slice(0, 10), c: 흐림 },
        { s: fn(r.name), sub: fi(r.inst) },
        { s: 쉼표(r.n), a: 'r' },
        { s: `⟦${쉼표(r.amt)}⟧`, a: 'r' },
        { s: r.my_rate != null ? `${Number(r.my_rate).toFixed(3)}%` : '—', a: 'r' },
        { s: r.over_pp != null ? pp(r.over_pp) : '—', a: 'r' },
      ]), Object.assign([0.13, 0.38, 0.08, 0.17, 0.12, 0.12], { a: ['l', 'l', 'r', 'r', 'r', 'r'] }))
    g.글('«하한 위»는 그 금액이 낙찰하한선보다 몇 %p 위였나입니다 — *딸 때 겨눈 자리*입니다.',
         { pt: 8.5, 색: 흐림, 위: PT(6) })
  }

  /* ── 개찰 하나하나 ───────────────────────────────────── */
  표제(g, `개찰 하나하나 — ${R.length}건 전부`)
  ...
  /* ── 그래서 무엇부터 ─────────────────────────────────────
     ⚠️ 여기서 «이렇게 하면 딴다» 고 적지 않습니다. 딸 확률은 금액으로 안 움직입니다.
        적을 수 있는 것은 «지금 새고 있는 곳» 뿐이고, 그것도 이 회사 숫자에서만 끌어옵니다.
        숫자가 없는 조언은 한 줄도 넣지 않습니다. */
  const 할것 = []
  if (dqa && dqa.건 >= 2) {
    할것.push(`*하한선 아래로 쓴 ${dqa.건}건* 부터 막으십시오. 등수에서 진 게 아니라 ` +
      `심사에 못 올라간 자리입니다. 가운데값으로 *${Number(dqa.모자란pp중앙).toFixed(3)}%p* ` +
      '모자랐습니다 — 이만큼만 올렸어도 살아남을 자리였습니다.')
  }
  if (참가.size >= 2) {
    const 많은칸 = ['30~99곳', '100곳 이상'].filter((k) => 참가.has(k))
    const 많은투 = 많은칸.reduce((a, k) => a + 참가.get(k).투찰, 0)
    if (많은투 >= 3) {
      할것.push(`투찰 ${R.length}건 중 *${많은투}건*이 30곳 넘게 붙는 자리였습니다. ` +
        '거기서는 금액을 아무리 다듬어도 확률이 안 올라갑니다 — *사람이 덜 붙는 공고를 고르는 것*이 먼저입니다. ' +
        '바로투찰 화면이 공고마다 «예상 참가» 를 적어 드립니다.')
    }
  }
  if (hb2 && hb2.낙찰선중앙 != null && hb2.흔들림 > Math.abs(hb2.낙찰선중앙) * 2) {
    할것.push(`금액이 회마다 *±${Number(hb2.흔들림).toFixed(3)}%p* 씩 흩어집니다. ` +
      `낙찰선은 하한 위 ${pp(hb2.낙찰선중앙)} 자리에 있습니다 — *금액 정하는 방법을 하나로 고정*하시면 ` +
      '겨냥은 그대로 두고 흔들림만 줄일 수 있습니다.')
  }
  const 놓친것 = (d.놓친자리 || []).filter((m) => m.enough && m.room != null && m.room >= m.gap)
  if (놓친것.length) {
    할것.push(`*${놓친것.length}건*은 하한선까지 여유가 남아 있는데 1순위에 밀렸습니다 — ` +
      '「놓친 자리 다섯」 을 보시면 얼마를 더 낮출 수 있었는지 원 단위로 적어 두었습니다.')
  }
  if (할것.length) {
    표제(g, '그래서 무엇부터 — 이 종이가 말하는 것', true)
    할것.forEach((t, i) => g.글(`${i + 1}. ${t}`, { pt: 10.5, 위: i ? PT(6) : 0, 들여: PT(4) }))
    g.글('⚠️ 어느 것도 «이렇게 하면 딴다» 는 말이 아닙니다. 3년치 실측으로는 금액을 어떻게 잡아도 ' +
         '1순위율이 3.5~4.4%에서 움직이지 않습니다. 위 넷은 *지금 새고 있는 곳*입니다 — ' +
         '새는 것부터 막는 편이 확실합니다.', { pt: 8.5, 색: 흐림, 위: PT(8) })
  }
...
/* ══════════════════════════════════════════════════════════════
   종이에서만 쓰는 잔셈 — ⚠️ **새 식을 여기 적지 않습니다.**
   성적표.js 가 한 건씩 채점해 둔 줄(_전부)을 «묶어 세기만» 합니다.
   여기서 하한금액이나 투찰률을 다시 계산하면 두 벌이 조용히 어긋납니다.
   ══════════════════════════════════════════════════════════════ */
export const 참가칸 = (n) => (
  (n || 0) <= 1 ? '혼자 (경쟁 없음)'
    : n <= 9 ? '2~9곳'
    : n <= 29 ? '10~29곳'
    : n <= 99 ? '30~99곳' : '100곳 이상')
const 칸차례 = ['혼자 (경쟁 없음)', '2~9곳', '10~29곳', '30~99곳', '100곳 이상']
...
function 묶어세기(줄들, 열쇠) {
  ...
  for (const x of 줄들) {
    ...
    if (!m.has(k)) m.set(k, { 투찰: 0, 낙찰: 0, 실격: 0, 등수합: 0 })
    const v = m.get(k)
    v.투찰 += 1
    v.등수합 += x.rank
    if (x.rank === 1) v.낙찰 += 1
...
/** 「금액을 조금 움직였다면」 — 내 모든 투찰을 예정가격의 d%p 만큼 옮겨 다시 셉니다.
 *  ⚠️ **남들 금액은 그대로**라고 보고 센 값입니다. 실제로는 남도 움직입니다.
 *     그래서 «그러면 이렇게 된다» 가 아니라 «내 금액이 어디쯤이었나» 를 보는 자입니다. */
function 되짚기(줄들, d) {
  let 잰것 = 0, 실격 = 0, 일순위 = 0
  for (const x of 줄들) {
    if (!x.limit || !x.yeje || !x.win_amt) continue
    잰것 += 1
    const amt = x.amt + (d / 100) * x.yeje
    if (amt < x.limit) 실격 += 1
    else if (amt <= x.win_amt) 일순위 += 1
  }
  return { 잰것, 실격, 일순위 }
}
