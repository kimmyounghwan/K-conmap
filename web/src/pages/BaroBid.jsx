...
export default function BaroBid() {
  ...
  const pick = (r) => {
    ...
    setAAuto(false)
    setSjPick(null)
    /* 💬 2026-09-17 — 「바로투찰을 한 번 썼다」 는 **여기** 입니다.
     *
     * ⚠️ 전에는 「💰 금액 복사하고 나라장터 열기」 에만 달려 있었습니다. 그러면
     *    **하루에 실제 투찰을 다섯 번** 해야 창이 떴습니다 — 그런 사람은 거의 없습니다.
     *    소장님: 「바로입찰 6번 눌렀는데도 글쓰라는 창이 안뜨던데」 — 그래서였습니다.
     *
     * ⚠️ 그렇다고 «화면을 연 횟수» 로 세면 안 됩니다. 바로투찰은 **첫 화면(/)** 이라
     *    그건 「방문 수」 입니다. 아무것도 못 받은 사람에게 «한 줄 남겨 달라» 는 꼴이 됩니다.
     *
     * → 「공고 하나를 골라 투찰금액을 봤다」 를 셉니다. 그때 그 사람은 **답을 손에 쥡니다.**
     *   공고·1순위 목록의 「💰 바로투찰」 로 들어와도(?no=…), 여기서 검색해 골라도 같습니다.
     *   ⚠️ 기초금액만 손으로 넣는 경우는 세지 않습니다 — 공고를 고른 것이 아닙니다.
     *   다섯 번째에 그 날 한 번 뜹니다 (AskComment.jsx 의 표). */
  ...
  const copyAndGo = () => {
    ...
    /* ⚠️ 2026-09-17 — 여기 있던 askAfter('bid') 를 pick() 으로 옮겼습니다.
       여기에 두면 «실제로 투찰하러 간 횟수» 를 세는 것이라 다섯 번을 채우는 사람이 없었습니다.
       그리고 여기는 pick() «뒤» 라, 남겨 두면 같은 공고를 두 번 세게 됩니다. */
  }
    ...
    <>
      ...
      {picked && !isReady(picked) && (
        <div className="warnbox nogobox">
          ...
          <p className="last">
            보통 <b>기초금액이 공개되면</b> 나머지도 같이 들어옵니다. 자동 갱신 때마다 다시 받아오니
      ...
      {picked && !isReady(picked) ? null : base > 0 && band && band.rec == null ? (
        ...
        <>
          ...
          <div className={'hero ' + (pass === false ? 'bad' : '')}>
            ...
            <div className="hbtns">
              ...
              {/* ⚠️ 2026-09-06 — 여기 있던 「나라장터 공고 →」 를 뺐습니다.
                  소장님: 「왜 나라장터 공고로 가지? 금액이 나오면 투찰할 수 있는 창으로 가야지.
                           바로투찰에서는 … 이미 다른 데에 공고보기가 많이 있잖아」 — 맞습니다.
                  이 줄은 «금액이 나온 뒤» 의 자리라 다음 행동은 «투찰» 하나뿐입니다.
                  공고 보기는 위 「나라장터에서 이 공고 열기」, 계산 불가 안내,
                  공고 탭·1순위·/notice 에 이미 있습니다. 같은 화면에 나라장터 버튼이 둘이면
                  «공고» 쪽을 누르게 되고, 그건 투찰이 아닙니다.
                  → 투찰 버튼(.gobid) 하나만 바로 아래에 둡니다. */}
            </div>
          ...
          {/* 투찰 바로가기 — 바로투찰 화면의 «유일한» 나라장터 버튼입니다.
              ⚠️ 주소는 나라장터 입구(www.g2b.go.kr) 입니다. «로그인 화면» 을 바로 여는 주소는
                 확인하지 못했습니다 — 나라장터는 robots 로 자동 확인을 막고 있어 긁지 않습니다
                 (고용24 때와 같은 원칙). 화면 번호를 짐작해 주소를 만들면 차수 000 사고의 재발입니다.
                 소장님이 로그인하실 때 주소창 주소를 알려 주시면 그때 정확히 바꿉니다.
...
/* ============================================================
   ...
   한계를 숨기지 않습니다. 다만 **2026-09-06 에 그 한계를 다시 쟀습니다** —
   예전에는 «조달청이 1순위만 주기 때문에» 라고 적었는데 그건 틀린 말이었습니다
   (조달청은 개찰 순위를 줍니다). 실제로 남는 한계는 우리 회사의 적격심사 점수입니다.
   ============================================================ */
function SimBlock({ bt, open, setOpen }) {
  ...
  return (
    <div className="card c-sim" id="sim">
      ...
      {/* ★ 2026-09-06 — 이 문단은 틀린 말이었습니다.
          「조달청이 1순위(낙찰자)만 알려주기 때문에 … 2위 이하 업체와의 경쟁을 반영하지 못했습니다」
          ① 조달청은 **개찰 순위를 줍니다** (공고번호로 물으면 옵니다 — CLAUDE.md 2026-09-02).
          ② 그리고 «2위 이하와의 경쟁» 은 애초에 판정에 필요 없습니다. 1순위는 낙찰하한을
             넘긴 것 중 가장 싼 투찰입니다. 우리가 그보다 싸면서 하한을 넘겼으면 우리가 가장
             쌉니다 — 2위 이하는 우리보다 위이거나 하한 아래입니다.
          진짜로 남는 한계는 **우리 회사의 적격심사 점수**입니다. 그래서 그것만 적습니다.
          숫자(qn·qs)는 build_json 이 실측해 넣습니다 — 상수로 박지 않습니다. */}
      <div className="note sm">
        <b>이 숫자를 그대로 믿지 마세요.</b> 여기까지는 <b>가격</b> 이야기입니다 —
        «낙찰하한을 넘겼고 실제 1순위보다 쌌나»를 대본 것입니다.
        {bt.qn > 0 && (
          <> 순위까지 받아 둔 개찰 {num(bt.qn)}건으로 재보니, 1순위보다 싸면 실제로도
            가장 싼 투찰이 되는 경우가 {((1 - bt.qs / bt.qn) * 100).toFixed(1)}%
            였습니다(나머지 {num(bt.qs)}건은 더 싸게 쓴 곳이 적격심사에서 밀린 경우입니다).</>
        )}
        {' '}반영하지 <b>못한</b> 것은 <b>우리 회사의 적격심사 점수</b>(경영상태·시공경험·
        신인도)입니다. 가격에서 1순위가 되어도 적격심사에서 떨어지면 낙찰이 아닙니다.
        {' '}그만큼 <b>실제 승률은 위 숫자보다 낮습니다.</b>
      </div>
    </div>
  )
}
